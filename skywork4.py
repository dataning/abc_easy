...
# Check for httpx availability (needed for Cerebras proxy support)
try:
    import httpx
    HTTPX_AVAILABLE = True
    print("✅ httpx available for corporate proxy support")
except ImportError:
    HTTPX_AVAILABLE = False
    print("⚠️ httpx not available - Cerebras proxy support limited")
...
def test_proxy_connection():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Proxy working! External IP: {result.get('origin', 'Unknown')}")
            return True
        else:
...
def create_corporate_proxy_http_client():
    """Create HTTP client that works with corporate proxy"""
    if HTTPX_AVAILABLE:
        try:
            client = httpx.Client(
            ...
            print("✅ Corporate proxy HTTP client created")
            return client
        except Exception as e:
            print(f"⚠️ Could not create corporate proxy HTTP client: {e}")
            return None
    else:
        print("⚠️ httpx not available for corporate proxy support")
        return None

# ========================= UPDATED GENERIC LLM CLIENT WITH PROXY SUPPORT =========================

class GenericLLMClient:
    """Generic LLM client updated for latest API specifications with proxy support"""
    ...
    def _init_client(self):
        """Initialize the appropriate client based on provider with proxy support"""
        if self.provider == "cerebras":
            from cerebras.cloud.sdk import Cerebras
            api_key = os.environ.get("CEREBRAS_API_KEY", "csk-y2kfmetpm2kww44t6emmc656x8kr49j8pf3krvnjj3twt525")
            
            # Try to create proxy-aware client if httpx is available
            if HTTPX_AVAILABLE:
                self.proxy_http_client = create_corporate_proxy_http_client()
                if self.proxy_http_client:
                    return Cerebras(api_key=api_key, http_client=self.proxy_http_client)
                else:
                    print("⚠️ Falling back to standard Cerebras client")
            ...
            return Groq(api_key=os.environ.get("GROQ_API_KEY", "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"))
        
        elif self.provider == "gemini":
            from google import genai
            api_key = os.environ.get("GEMINI_API_KEY", "AIzaSyC_kJEjiBGbGqOMshzsLC5DyDtutSNoZpQ")
    ...
    def _get_model(self) -> str:
        ...
        return models.get(self.provider, "default-model")
    ...
    def generate(self, prompt: str, system_prompt: str = "", temperature: float = 0.1, 
        ...
        """Universal generate method updated for latest APIs with proxy error handling"""
        try:
            # Cerebras - Updated API format
            if self.provider == "cerebras":
                messages = []
                if system_prompt:
            ...
            if "proxy" in error_str or "connection" in error_str or "ssl" in error_str:
                print(f"⚠️ Proxy-related error with {self.provider.title()}: {e}")
                print(f"   Proxy settings: {os.environ.get('HTTPS_PROXY', 'Not set')}")
                
                # Try to diagnose the issue
                print(f"   Running proxy diagnostics...")
                test_proxy_connection()
            else:
                print(f"❌ {self.provider.title()} API error: {e}")
            return f"Error: {str(e)}"
    
    def generate_structured(self, prompt: str, schema: Dict, system_prompt: str = "", 
                          temperature: float = 0.1, max_tokens: int = 3000) -> Dict:
        """Generate structured output with proper schema handling per provider"""
        try:
            if self.provider == "cerebras":
                messages = []
                if system_prompt:
                ...
                # Fall back to JSON object mode for these providers
                messages = []
                if system_prompt:
                    messages.append({"role": "system", "content": system_prompt})
                ...
                full_prompt = f"{system_prompt}\n\n{prompt}\n\nReturn valid JSON matching the schema." if system_prompt else f"{prompt}\n\nReturn valid JSON matching the schema."
            ...
            if "proxy" in str(e).lower() or "connection" in str(e).lower():
                print(f"   This appears to be a proxy-related error")
                test_proxy_connection()
            
            # Fallback to regular generation and try to parse JSON
            try:
                response_text = self.generate(prompt, system_prompt, temperature, max_tokens)
...
def test_api_with_proxy(llm_client: GenericLLMClient):
    """Test API calls through proxy"""
    print(f"🧪 Testing {llm_client.provider.title()} API through corporate proxy...")
    ...
    try:
        ...
        if "Error" not in response:
            print(f"✅ {llm_client.provider.title()} API working through corporate proxy!")
            print(f"   Response preview: {response[:50]}...")
            return True
        else:
            print(f"❌ {llm_client.provider.title()} API failed: {response}")
            return False
    except Exception as e:
        print(f"❌ API test failed: {e}")
        return False

# Initialize the LLM client with proxy support
print("\n🚀 Initializing LLM client with corporate proxy support...")
llm_client = GenericLLMClient("cerebras")  # Options: "cerebras", "openai", "groq", "gemini"
...
if proxy_ok and api_ok:
    print("\n🎉 All systems ready with corporate proxy support!")
else:
    print("\n⚠️ Some connectivity issues detected but continuing...")
...
def extract_knowledge_graph_with_llm(query: str) -> Dict:
    """Extract entities, nodes, and generate search terms using generic LLM"""
    kg_schema = {
        "type": "object",
        "properties": {
            "entities": {
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(future_to_term):
            ...
            try:
                ...
                if not df.empty:
                    dfs.append(df)
                    print(f"  ✓ {term}: {len(df)} articles")
                else:
                    print(f"  ✗ {term}: no articles")
            except Exception as e:
                print(f"  ✗ {term}: {e}")
...
def translate_headline_to_english(headline: str, source_language: str) -> str:
    """Translate non-English headlines to English using generic LLM"""
...
def judge_headline_relevance(headline: str, query: str) -> Dict:
    """Use generic LLM to judge the relevance of a headline to the query"""
    schema = {
        "type": "object",
        "properties": {
...
def generate_summary_with_llm(df: pd.DataFrame, query: str) -> str:
    ...
    if len(df) > 5:
        print(f"📊 {len(df)} articles available - applying score_geo > 0 filter")
        df_relevant = df[df['score_geo'] > 0].copy()
        
        if df_relevant.empty:
            print("⚠️ No articles with score_geo > 0, using all available articles")
            df_relevant = df.copy()
        else:
            print(f"✅ Filtered to {len(df_relevant)} articles with score_geo > 0")
    else:
        print(f"📊 Only {len(df)} articles available - using all articles for summary generation")
        df_relevant = df.copy()
    ...
    print(f"📝 Using {len(df_top)} articles for summary generation")
    ...
    try:
        print(f"🔄 Generating summary with {llm_client.provider} model: {llm_client.model}")
        print(f"📊 Articles in prompt: {len(df_top)}")
        print(f"📝 Prompt length: {len(prompt)} characters")
        
        # Use the generic LLM client's structured generation method
        ...
        if not result:
            print("❌ No result from structured generation")
        ...
        print(f"📅 Timeline events count: {len(result.get('timeline', []))}")
...
"""
        ...
        print(f"🔗 Found {len(cited_ids)} citations in answer: {cited_ids}")
        
        if cited_ids:
            summary += f"""
📋 CITED SOURCES ({len(set(cited_ids))} unique sources):
"""
            ...
            for i, news_id in enumerate(unique_cited_ids, 1):
                if news_id in news_id_mapping:
                    ...
                    print(f"⚠️ Citation {news_id} not found in news_id_mapping")
        else:
            print("ℹ️ No citations found in answer, showing top sources")
            summary += f"""
📋 KEY SOURCES (Top 5 by relevance):
"""
...
📈 ANALYSIS METADATA:
• Query: {query}
• Data Processing: {filtering_note}
• Articles Analyzed: {len(df_top)} articles for summary generation
• Total Articles Available: {len(df)} articles
• Confidence Level: {result['confidence_level']}
• Date Range: {df['Date'].min()} to {df['Date'].max()}
• Average Relevance Score: {df_top['score_geo'].mean():.3f}
• Sources Actually Cited: {len(set(cited_ids))} unique citations
• LLM Provider: {llm_client.provider} ({llm_client.model})
• Proxy Status: {'Active' if os.environ.get('HTTPS_PROXY') else 'Not configured'}
"""
        ...
        print("🔄 Falling back to conservative summary...")
        return create_conservative_fallback_summary(df, query, news_id_mapping, df_top)
...
"""
    ...
    for idx, row in df_top.head(5).iterrows():
        ...
        fallback_summary += f"• {row['Date']}: {headline_preview}... ({row['domain']})\n"
...
"""
    ...
    for idx, row in df_top.head(5).iterrows():
        ...
        fallback_summary += f"• [{headline_preview}...]({url}) ({row['domain']}, {row['Date']}) - Score: {score:.3f}\n"
...
📈 ANALYSIS METADATA:
• Query: {query}
• Headlines Analyzed: {len(df)} total, {len(df_top)} most relevant
• Query-Specific Matches: {specific_matches} headlines
• Date Range: {min_date} to {max_date} ({date_range_days} days)
• Top Sources: {', '.join(top_domains) if top_domains else 'Various sources'}
• Average Relevance: {avg_relevance:.3f}
• Confidence Level: {'Medium' if specific_matches > len(df_top) * 0.3 else 'Low'} (headline-only analysis, {'specific data found' if specific_matches > 0 else 'limited specific data'})
• Data Limitation: Analysis based on headlines only - full article content needed for detailed information
• Focus Area: {focus_area}
• Main Entity: {main_entity}
• Search Terms Used: {len(query_terms)} terms derived from query
• Recommendation: {'Expand search terms' if specific_matches == 0 else 'Search specialized publications'} for {focus_area.replace(' activities and developments', '')} information
• Proxy Status: {'Active' if os.environ.get('HTTPS_PROXY') else 'Not configured'}
"""
